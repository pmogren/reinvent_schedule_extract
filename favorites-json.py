...
for session in session_data:
    event = ObjDict()
    event.summary = session['schedule_status'] + session['abbreviation'] + session['title'].replace("'","\'")
    event.description = session['abstract']
    event.location = session['schedule']['room']
    event.dtstart = session['schedule']['startDatetime']
    event.dtend = session['schedule']['endDatetime']
    event.url = base_url + "/sessionDetail.ww?SESSION_ID=" + session['normalized_abbreviation'].replace(" - ", "")
    event.dtstamp = session['schedule']['startDatetime']
    cal.favorites.append(event)

# write the json file
with open("reinvent.json","w") as f:
    f.write(cal.dumps())
